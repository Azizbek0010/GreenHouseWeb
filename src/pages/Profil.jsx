  ...
  return (
    <div>
      ...
      >
        ...
        <ChevronDown size={16} className={`text-text-sub transition-transform duration-200 shrink-0 ${open ? 'rotate-180' : ''}`} />
      </button>
...
function LoginForm({ user, onSave }) {
  ...
  return (
    <div className="space-y-3 pt-2">
      <div>
        <label className="text-xs font-semibold text-text-sub uppercase tracking-wide block mb-1.5">Yangi telefon raqam</label>
...
export default function Profil() {
  ...
  return (
    <div className="p-4 md:p-6 max-w-xl mx-auto">
      ...
      <h1 className="text-2xl font-bold text-ctext tracking-tight mb-5">Profil</h1>
      ...
      <div className="bg-ccard border border-cborder rounded-2xl overflow-hidden mb-5">
        <AccordionRow icon={Phone} label="Login o'zgartirish" sublabel={user?.phone || '+998 — — — —'}>
          <LoginForm user={user} onSave={(u) => updateUser(u)} />
        </AccordionRow>
        <div className="h-px bg-separator" />
        <AccordionRow icon={Lock} label="Parol o'zgartirish" sublabel="Hisob xavfsizligini oshiring">
          <PasswordForm />
        </AccordionRow>
      </div>
      ...
      <div className="bg-ccard border border-cborder rounded-2xl overflow-hidden mb-5">
        <div className="flex items-center justify-between px-4 py-4">
          ...
          <button onClick={toggle} className={`relative w-14 h-7 rounded-full transition-colors ${dark ? 'bg-primary' : 'bg-[#d1d5db]'}`}>
            <span
      ...
      <button
        ...
        className="flex items-center justify-center gap-2 w-full h-12 rounded-2xl border border-cred text-cred text-sm font-semibold hover:bg-red-bg transition-colors"
      >
